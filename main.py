...
def newScreenHelper(screen, width, height, fontSize, text, textColor, resizeWidth, resizeHeight, fill):
    ...
    screen.blit(surface, ((WIDTH-surface.get_width())/resizeWidth, (HEIGHT -
                                                                    surface.get_height())/resizeHeight))  # position surface onto screen
    return screen
...
def gameOverScreen(screen):
    ...
    screen = newScreenHelper(screen, WIDTH/2, HEIGHT/5, 100,
                             "GAME OVER!", (255, 255, 255), 2, 4, None)  # game over
    screen = newScreenHelper(screen, WIDTH/4, HEIGHT/10, 50,
                             "PLAY AGAIN", (255, 255, 255), 2, 2, (0, 255, 0))  # play again
    screen = newScreenHelper(screen, WIDTH/4, HEIGHT/10, 50,
                             "QUIT", (255, 255, 255), 2, 3/2, (0, 0, 0))  # quit
...
def pauseScreen(screen):
    pygame.draw.rect(screen, (255, 255, 255), (WIDTH/4, HEIGHT/6, WIDTH/2,
                                               HEIGHT/(3/2)), border_radius=int(min(WIDTH/2, HEIGHT/(3/2))/4))
    screen = newScreenHelper(screen, WIDTH/2, HEIGHT/5,
                             100, "PAUSED", (0, 0, 0), 2, 4, None)  # game over
    screen = newScreenHelper(screen, WIDTH/4, HEIGHT/10, 50,
                             "RESUME", (255, 255, 255), 2, 2, (0, 255, 0))  # play again
    screen = newScreenHelper(screen, WIDTH/4, HEIGHT/10, 50,
                             "QUIT", (255, 255, 255), 2, 3/2, (0, 0, 0))  # quit
...
kaytonf: the-floating-dutchman / (252ddd8✗) $
            ...
            if ((HEIGHT*(11/20)) >= mouse[1] >= (HEIGHT*(9/20)) or ev == 0):
                screen = newScreenHelper(
                    screen, WIDTH/4, HEIGHT/10, 50, text, (255, 255, 255), 2, 2, (0, 255, 0))  # play again
                screen = newScreenHelper(
                    screen, WIDTH/4, HEIGHT/10, 50, "QUIT", (255, 255, 255), 2, 3/2, (0, 0, 0))  # quit
                playAgain = True
                mouseDown = True
            # user hovers over quit button
            elif ((HEIGHT*(7/10)) >= mouse[1] >= (HEIGHT*(3/5)) or ev == 1):
                screen = newScreenHelper(
                    screen, WIDTH/4, HEIGHT/10, 50, "QUIT", (255, 255, 255), 2, 3/2, 'red')  # quit
                screen = newScreenHelper(
                    screen, WIDTH/4, HEIGHT/10, 50, text, (255, 255, 255), 2, 2, (0, 0, 0))  # play again
                playAgain = False
                mouseDown = True
        pygame.display.update()  # update screen
